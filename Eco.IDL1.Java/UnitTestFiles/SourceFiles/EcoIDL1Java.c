...
int16_t EcoMain(IEcoUnknown* pIUnk) {
    ...
    result = pIBus->pVTbl->QueryComponent(pIBus, &CID_EcoIDL1Java, 0, &IID_IEcoIDL1Java, (void**) &pIEcoIDL1Java);
    if (result != 0 || pIEcoIDL1Java == 0) {
        /* Free interfaces in case of an error */
        goto Release;
    }


    result = pIEcoIDL1Java->pVTbl->MyFunction(pIEcoIDL1Java, name, &copyName);
...
Release:
    ...
    if (pIEcoIDL1Java != 0) {
        pIEcoIDL1Java->pVTbl->Release(pIEcoIDL1Java);
    }
