...
int16_t EcoMain(IEcoUnknown* pIUnk) {
    ...
    result = pIBus->pVTbl->QueryComponent(pIBus, &CID_EcoIDL1CPP, 0, &IID_IEcoIDL1CPP, (void**) &pIEcoIDL1CPP);
    if (result != 0 || pIEcoIDL1CPP == 0) {
        /* Free interfaces in case of an error */
        goto Release;
    }


    result = pIEcoIDL1CPP->pVTbl->MyFunction(pIEcoIDL1CPP, name, &copyName);
...
Release:
    ...
    if (pIEcoIDL1CPP != 0) {
        pIEcoIDL1CPP->pVTbl->Release(pIEcoIDL1CPP);
    }
